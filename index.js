...
check.addEventListener("click",function(){
    ...
    if (input.value == name1.toLowerCase){
 ...
 letter.forEach(button => {
    button.addEventListener('click', function() {
        ...
        console.log(name1);
        if (name1.includes(button.value)){
            console.log(name1);
            letterindex = name1.indexOf(button.value);
            let divElement = document.getElementById(`div-${letterindex + 1}`);
            divElement.textContent = button.value;
            let count = name1.split('').filter(char => char === button.value).length;
            tryword = tryword + button.value.repeat(count);
            if (count > 1){
                for(let i = 1;i<count;i++){
                    let index = letterindex + 1;
                    letterindex = name1.indexOf(button.value, index);
...
function checkentry(){
    if ( tryword.length == name1.length){
                win.style.visibility = "visible";
                main.style.visibility = "hidden";
                lettertry = 0;
    }
        ...
        if (name1.includes(" ")){
            if ( tryword.length == name1.length - 1){
                win.style.visibility = "visible";
                main.style.visibility = "hidden";
                lettertry = 0;
            }
